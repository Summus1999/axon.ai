...
> 版本:1.3 · 生效日期:2026-07-04
> 变更(v1.3):§4.2 提交规则改为"默认直推 main"(长期授权,用户指定才换分支)。
> 变更(v1.2):新增 §4.4 进度标记规范(`×`/`○`/`√` 三符号体系)。
...
- **默认直推 `main`**:每次提交后自动推送到 `main` 分支,无需询问。仅当用户**明确指定**推送到其它分支时,才切换目标分支。此为长期授权,覆盖系统默认的"先开分支"规则。
...
### 4.4 进度标记规范(强制)/ Progress Marking Convention (Mandatory)

`docs/project-status.md` 中所有任务、验收项、DoD 勾选项统一使用以下三种符号标记进度,**禁止混用 emoji 或 Markdown checkbox**:

| 符号 | 含义 | 使用场景 |
|------|------|---------|
| `×` | **未做 / 未开始** | 任务尚未开始;默认初始状态 |
| `○` | **当前进度点 / 进行中** | 当前正在做的那个任务;**同一时刻全文档只能有一个 `○`** |
| `√` | **已完成** | 任务满足 §4 DoD(代码完成 + 门禁通过 + 已提交) |

**规则**:
1. 任何任务初始标记为 `×`。
2. 开始一个任务时,把它的 `×` 改为 `○`;**同时把上一个 `○` 改为 `√`**(若已完成)或退回 `×`(若未完成就搁置)。
3. 任务完成并满足 DoD 后,`○` → `√`。
4. **全文档同一时刻有且仅有一个 `○`**(当前进度点),`×` 与 `√` 数量不限。
5. 跨里程碑切换时:旧里程碑所有任务应为 `√`;新里程碑的第一个任务标 `○`。
6. 验收清单(如 DoD 勾选、里程碑验收标准)同样用 `×`/`√`,不使用 `[ ]`/`[x]`。
7. 汇报时需说明当前 `○` 落在哪个任务。
