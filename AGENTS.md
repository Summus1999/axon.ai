...
> 版本:1.1 · 生效日期:2026-07-04
> 变更(v1.1):新增"开发前必读进度文档"强制规矩(§0 第 3 条 + §4.1 步骤 1/7)。
...
2. **三条铁律不可破坏**(详见 §1–§3):
   ...
   - 代码分块必须合理
   - 需求先写测试,提交前测试必须全过
3. **开发前必读进度文档(强制)**:每次开始任何开发任务前,必须先完整阅读
   [`docs/project-status.md`](./docs/project-status.md),掌握:
   - 当前里程碑与进度快照(§2、§5)
   - 当前里程碑的任务看板与依赖顺序(§6)
   - 任务状态模型与 Definition of Done(§3、§4)
   - 风险与阻塞登记(§7)
   不读进度文档就开工,视为违规;不得凭记忆假设进度。
4. **诚实汇报**:测试失败就说失败、贴输出;跳过步骤就说跳过;只有验证通过才说"完成"。
5. **不可逆动作(提交/推送/删除/覆盖)先确认**,除非用户已明确授权;授权不跨场景。
...
1. **读进度文档(强制首发)**:完整阅读 `docs/project-status.md`,确认当前里程碑、待办任务、依赖顺序、阻塞与风险(见 §0 第 3 条)。然后读代码、搜现有实现,优先复用;不确定先问。
2. **规划**:非简单任务进 plan mode,写计划待审批;计划中指明本次开发对应进度文档里的哪个任务 ID。
3. **测试先行**:按 §3.1 写测试(Red)。
4. **实现**:最小实现让测试通过(Green),遵守 §1–§2。
5. **重构**:优化代码,测试仍绿。
6. **验证**:跑 §3.4 全套门禁。
7. **更新进度**:在 `docs/project-status.md` 更新对应任务状态 + 关联 commit hash(见 project-status §8.1)。
8. **汇报**:如实说明做了什么、验证结果、进度文档更新情况、需注意点。
