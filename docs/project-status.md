...
```
M0  √  100%  已完成
M1  ○   93%  当前进度点  ← M1-T13 已完成，剩余 T14/T15
M2  ×    0%  未做
M3  ×    0%  未做
M4  ×    0%  未做
M5  ×    0%  未做
...
| 无真实测试 | 骨架阶段 0 测试,M1 起按测试先行补充 | 由 M1-T14 真实 DeepSeek API 端到端测试解决(进行中) |
...
| **M1-T13** | 配置加载:figment 接入 toml+env | √ | 待提交 |
| **M1-T14** | 端到端集成测试(真实 DeepSeek API + Docker 隔离) | ○ | — |
| **M1-T15** | M1 验收 + 文档同步 + 用户 review | × | — |

> 状态符号:`×` 未做 / `○` 当前进度点(全文档唯一)/ `√` 已完成。当前 `○` 落在 **M1-T14**。
...
- ○ `axon run --goal "写一个返回 hello world 的函数"` 使用真实 DeepSeek API 端到端成功
