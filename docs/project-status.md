...
| **M1** | v0.2 | 单机 MVP:CLI + OpenAI/DeepSeek + Docker 隔离 + 自检 | ○(当前) | — |
| **M2** | v0.3 | 记忆系统:Qdrant + redb + 用户画像 + 记忆管理 | × | — |
...
```
M0  √  100%  已完成
M1  ○    7%  当前进度点  ← M1-T1 已完成，M1-T2 取消，M1-T3 进行中
M2  ×    0%  未做
M3  ×    0%  未做
M4  ×    0%  未做
M5  ×    0%  未做
...
| **M1-T6** | Dispatcher:并发调度循环 + 重试/超时(S3.2.1/S3.2.2) | × | — |
| **M1-T7** | Isolation:DockerProvider 启停容器(S3.3.1) | × | — |
| **M1-T8** | Isolation:容器内执行命令 + 工作目录挂载(S3.3.2) | × | — |
| **M1-T9** | Worker:接收任务 + 调 LLM 生成改动(S4.1.1/S4.1.2/S4.1.3) | × | — |
| **M1-T10** | Worker:自检验收(跑测试/构建)(S4.2.1/S4.2.2) | × | — |
| **M1-T11** | Worker:结果回报(S4.3.1) | × | — |
| **M1-T12** | CLI:`axon run` / `axon tasks` 接入真实流程(S5.1.1/S5.1.2) | × | — |
| **M1-T13** | 配置加载:figment 接入 toml+env | × | — |
| **M1-T14** | 端到端集成测试(testcontainers + mock LLM) | × | — |
| **M1-T15** | M1 验收 + 文档同步 + 用户 review | × | — |

> 状态符号:`×` 未做 / `○` 当前进度点(全文档唯一)/ `√` 已完成。当前 `○` 落在 **M1-T3**。
