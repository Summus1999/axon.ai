...
- **遇阻塞时**:立即登记到 §7,标 `🚫 Blocked` 并说明阻塞原因。
- **人工 review 节点**:每个里程碑结束需用户 review,通过方可标记 Done。
...
| 里程碑 | 版本 | 目标 | 状态 | 完成日期 |
|--------|------|------|------|---------|
| **M0** | v0.1 | 骨架:workspace + trait 占位 + 文档 | ✅ Done | 2026-07-04 |
| **M1** | v0.2 | 单机 MVP:CLI + OpenAI/Ollama + Docker 隔离 + 自检 | 📋 Todo | — |
| **M2** | v0.3 | 记忆系统:Qdrant + redb + 用户画像 + 记忆管理 | 📋 Todo | — |
| **M3** | v0.4 | Firecracker 强隔离 + VM 生命周期 + 快照 | 📋 Todo | — |
| **M4** | v0.5 | 分布式(NATS)+ Web 仪表盘 + 可观测性 | 📋 Todo | — |
| **M5** | v0.6+ | 高级:多步 DAG + ReAct + 多 agent + 成本监控 | 📋 Todo | — |
...
```
        ...
        └────┬───┬────┘
             ...
             ▼
        ┌──────────┐
        │   Done   │  满足 DoD + 用户 review(里程碑级)
        └──────────┘
```

**状态定义**:
- **📋 Todo**:已分解但未开始。
- **🔄 In Progress**:正在执行;同一里程碑同时 In Progress 的任务建议 ≤ 2,避免上下文切换。
- **✅ Done**:代码完成 + 测试通过(AGENTS.md §3.4 门禁)+ 关联提交。
- **🚫 Blocked**:无法继续,需外部输入(用户决策 / 依赖未就绪 / 环境问题)。必须登记 §7。
...
| 无真实测试 | 骨架阶段 0 测试,M1 起按测试先行补充 | M1 解决 |
...
| ID | 任务 | 状态 | 关联 |
|----|------|------|------|
| **M1-T1** | LLM Provider 抽象实现:OpenAI provider(S1.3.1/S1.3.2) | 📋 Todo | — |
| **M1-T2** | LLM Provider 抽象实现:Ollama provider(S1.3.1) | 📋 Todo | — |
| **M1-T3** | LlmRouter:按配置路由 provider(S1.3.1) | 📋 Todo | — |
| **M1-T4** | Brain:单步 Planner(Goal → 单 Task)(S1.1.1/S1.2.1) | 📋 Todo | — |
| **M1-T5** | Dispatcher:进程内优先级队列(S3.1.1) | 📋 Todo | — |
| **M1-T6** | Dispatcher:并发调度循环 + 重试/超时(S3.2.1/S3.2.2) | 📋 Todo | — |
| **M1-T7** | Isolation:DockerProvider 启停容器(S3.3.1) | 📋 Todo | — |
| **M1-T8** | Isolation:容器内执行命令 + 工作目录挂载(S3.3.2) | 📋 Todo | — |
| **M1-T9** | Worker:接收任务 + 调 LLM 生成改动(S4.1.1/S4.1.2/S4.1.3) | 📋 Todo | — |
| **M1-T10** | Worker:自检验收(跑测试/构建)(S4.2.1/S4.2.2) | 📋 Todo | — |
| **M1-T11** | Worker:结果回报(S4.3.1) | 📋 Todo | — |
| **M1-T12** | CLI:`axon run` / `axon tasks` 接入真实流程(S5.1.1/S5.1.2) | 📋 Todo | — |
| **M1-T13** | 配置加载:figment 接入 toml+env | 📋 Todo | — |
| **M1-T14** | 端到端集成测试(testcontainers + mock LLM) | 📋 Todo | — |
| **M1-T15** | M1 验收 + 文档同步 + 用户 review | 📋 Todo | — |
...
- [ ] `axon run --goal "写一个返回 hello world 的函数"` 端到端成功
- [ ] 任务在 Docker 容器内执行,主机无残留
- [ ] worker 自检(构建/测试)通过后才回报
- [ ] `axon tasks` 能查看任务状态流转
- [ ] M1 全部门禁(fmt/clippy/test)通过
- [ ] 用户 review 通过
