...
| R6 | `#[cfg(unix)]` 模块在 Windows 本地不编译 | Linux CI 回归被本地门禁漏掉 | 中 | M3 Linux CI 回归(`dead_code`/`type_complexity`/`default_constructed_unit_structs`)已修复;后续对 unix-only 模块的改动需用 `--target x86_64-unknown-linux-gnu` 交叉检查 | 已缓解 |
...
> 2026-07-05:M3 验收后 Linux CI 回归(`firecracker.rs` 的 `dead_code` + clippy `type_complexity`/`default_constructed_unit_structs`),因 `#[cfg(unix)]` 在 Windows 本地门禁漏掉。已修复(移除未构造的 `FcProcess`/`VmInstance` 死代码、提取 `ClientFactory` type alias、`SshExecBackend::default()` → `SshExecBackend`)。
