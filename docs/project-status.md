...
| R6 | `#[cfg(unix)]` 模块在 Windows 本地不编译 | Linux CI 回归被本地门禁漏掉 | 中 | M3 Linux CI 回归已修复;对 unix-only 模块的改动用 WSL Ubuntu 本地跑 `cargo clippy/test --workspace` 验证;Firecracker fixture 已提交进仓库(Git LFS)避免外部下载源失效 | 已缓解 |
...
> 验证:WSL Ubuntu 24.04 + KVM 本地跑 `firecracker_lifecycle_create_snapshot_destroy` 集成测试通过。
