...
9. [M4 任务分解 / M4 Breakdown](#9-m4-任务分解--m4-breakdown)
10. [风险与阻塞登记 / Risk & Blocker Log](#10-风险与阻塞登记--risk--blocker-log)
11. [文档维护规则 / Maintenance Rules](#11-文档维护规则--maintenance-rules)
...
| **M3** | v0.4 | Firecracker 强隔离 + VM 生命周期 + 快照 | √ | 2026-07-05 |
...
| **M2-T4** | 自动用户画像与语义记忆写入 | √ | `05c9d98` |
| **M2-T5** | 权重衰减与 LRU 短期记忆 | √ | `c0df045` |
| **M2-T6** | 记忆管理 CLI 完善与跨会话集成测试 | √ | `b19a5d5` |
| **M2-T7** | 文档同步、验收与用户 review | √ | `11dcf20` |

> 状态符号:`×` 未做 / `○` 当前进度点(全文档唯一)/ `√` 已完成。M2 已完成,等待 M3 启动。
...
| **M3-T5** | DockerProvider 资源限额补全(CPU/内存/网络) | √ | `50dcf2c` |
| **M3-T6** | Worker 心跳上报 | √ | `50dcf2c` |
| **M3-T7** | 调度大脑复核 Reviewer | √ | `50dcf2c` |
| **M3-T8** | `axon vms` CLI + VM 状态跟踪 | √ | `50dcf2c` |
| **M3-T9** | Linux CI Firecracker 集成测试 | √ | `daf75e1` |
| **M3-T10** | 文档同步、验收与用户 review | √ | `dc628fa` |

> 状态符号:`×` 未做 / `○` 当前进度点(全文档唯一)/ `√` 已完成。M3 已验收,当前 `○` 已转移至 M4-T1。
...
- √ `FirecrackerProvider` 在 Linux 上能启动 microVM、执行命令、销毁 VM(代码实现,真实启动验证依赖 Linux CI)。
...
- √ Linux CI 已配置 Firecracker 集成测试(运行结果见 Actions)。
- √ M3 全部门禁(fmt/clippy/test)通过。
- √ 用户 review 通过。

---

## 9. M4 任务分解 / M4 Breakdown

> M4 目标:实现 NATS 分布式队列,支持独立 worker 节点、远程 CLI 与 Web 仪表盘。
> 任务 ID 对齐 [product-roadmap.md §4](./product-roadmap.md#4-需求拆解--requirements-breakdown)。
...
| ID | 任务 | 状态 | 关联 |
|----|------|------|------|
| **M4-T1** | 新增 `axon-queue` crate：任务队列抽象 + InProcess/NATS 实现 | √ | `6fcedd0` |
| **M4-T2** | Dispatcher 远程节点：入队、结果聚合、心跳监听 | √ | `7f11196` |
| **M4-T3** | Worker 远程节点：注册、拉取任务、执行、发布结果/心跳 | √ | `f4ea20a` |
| **M4-T4** | 远程 CLI：`axon run --remote`、`axon tasks --remote`、`axon workers --remote` | √ | `d6ca893` |
| **M4-T5** | Web Dashboard：axum + WebSocket 实时推送 | √ | `35de865` |
| **M4-T6** | 可观测性：关键路径 tracing spans | √ | `6e047d8` |
| **M4-T7** | 集成测试：NATS server + 多 worker 并发执行 | ○ | — |
| **M4-T8** | 文档同步、验收与用户 review | × | — |

> 状态符号:`×` 未做 / `○` 当前进度点(全文档唯一)/ `√` 已完成。当前 `○` 落在 **M4-T4**。

### 9.2 建议执行顺序(依赖关系)

```
M4-T1(队列抽象 + NATS)───┬──► M4-T2(Dispatcher 远程节点)
                         ├──► M4-T3(Worker 远程节点)
                         ├──► M4-T4(远程 CLI)
                         └──► M4-T5(Web Dashboard)
                                  │
...
- √ `axon-queue` crate 编译通过,`InProcessQueue` 与 `NatsQueue` 均通过测试。
- × 1 dispatcher + 2 worker + NATS 场景下,`axon run --remote` 提交的任务被 worker 执行并返回结果。
- × `axon workers --remote` 能看到在线 worker 列表与心跳。
- √ Web Dashboard 能实时显示任务状态变化和 worker 心跳。
- × M4 全部门禁(fmt/clippy/test)通过。
- × 用户 review 通过。

---

## 10. 风险与阻塞登记 / Risk & Blocker Log
...
| R3 | Firecracker Rust SDK(`firec-rs`)成熟度 | M3 集成风险 | 中 | 已采用直接 REST API 方案,SDK 可替换 | 已缓解 |
| R4 | 记忆污染/上下文爆炸 | agent 质量下降 | 中 | 分层 + 权重衰减 + 用户调节 + Top-K | 已缓解 |
| R5 | 本地代理(7897)未运行 | 开发构建失败 | 高 | `CARGO_HTTP_PROXY=` 绕过;待定项目级配置 | 待用户决策 |
| R6 | `#[cfg(unix)]` 模块在 Windows 本地不编译 | Linux CI 回归被本地门禁漏掉 | 中 | M3 Linux CI 回归已修复;对 unix-only 模块的改动用 WSL Ubuntu 本地跑 `cargo clippy/test --workspace` 验证;Firecracker fixture 已提交进仓库(Git LFS)避免外部下载源失效 | 已缓解(2026-07-05 CI 全绿) |
...
> 2026-07-05:M3 验收后 Linux CI 回归。根因有两层:
> (1) `firecracker.rs` 的 `dead_code` + clippy `type_complexity`/`default_constructed_unit_structs`(因 `#[cfg(unix)]` 在 Windows 本地门禁漏掉)—— 已修复(移除 `FcProcess`/`VmInstance` 死代码、提取 `ClientFactory` type alias、`SshExecBackend::default()` → `SshExecBackend`)。
> (2) `firecracker integration` job 的 `Download Firecracker + vmlinux` 步骤 404 —— Firecracker 自 v1.3.0 起不再在 release 发布 vmlinux,旧 URL `vmlinux-5.10.209` 失效。改为将 fixture(firecracker 二进制 + Alpine vmlinux + rootfs.ext4)提交进仓库(Git LFS 跟踪)。
> 同时修复 Firecracker **v1.7.0 API 变更**:`--api-socket` → `--api-sock`、`ht_enabled` → `smt`、Pause/Resume 从 `PUT /actions` 改为 `PATCH /vm` + `{"state":"Paused"/"Resumed"}`、移除失效的 `--log-path`、修复 `fixture()` env var 名映射(`FIRECRACKER_BINARY`/`KERNEL_IMAGE`/`ROOTFS_IMAGE`)。
> 另修复:`axon-isolation/Cargo.toml` 显式声明 `tokio` `fs` feature(单 crate 编译时缺失);firecracker fixture 补回可执行权限位(`git update-index --chmod=+x`)。
> 验证:WSL Ubuntu 24.04 + KVM 本地跑 `firecracker_lifecycle_create_snapshot_destroy` 集成测试通过;GitHub Actions CI 全绿(check ubuntu/windows + firecracker integration)。

---

## 11. 文档维护规则 / Maintenance Rules

### 11.1 谁更新 / When to Update

| 事件 | 更新内容 | 责任 |
|------|---------|------|
| 任务状态变更 | §6/§7 看板状态 + 关联 commit | AI agent(执行后立即) |
| 遇阻塞 | §10.2 登记 + 任务标 Blocked | AI agent / 人类 |
